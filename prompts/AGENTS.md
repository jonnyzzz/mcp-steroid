...
**Escaping `${...}` inside `steroid_execute_code` script strings.** A `${...}` in a replacement
string passed to `VfsUtil.saveText` (or any nested Kotlin string literal in an exec_code example)
is interpolated by the OUTER script, not emitted literally. Break the template with a `val dollar = "$"` indirection — e.g.
