...
- The `updatePlugins.xml` is generated by `:site-gen:generateSiteArtifacts` (Kotlin). It resolves
  the release ZIP URL via the GitHub API, downloads it, and extracts `plugin.xml` from the JAR to
  get the exact version and `since-build`. No fallbacks — build fails on any error.
