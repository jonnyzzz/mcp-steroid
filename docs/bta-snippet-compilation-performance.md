# BTA snippet-compilation performance — PR #361 status and measurements
...
measurement history, not as an active follow-up for PR #361.
...
- Full design (shards, Building|Ready two-state, Windows-safe swap via new-dir + deferred
  GC, no daemon restarts) was drafted in the 2026-07-30 session.
...
These are the only remaining actions from this investigation:

1. Return the BTA API and implementation dependencies to the first regular Kotlin release
   containing [KT-87743](https://youtrack.jetbrains.com/issue/KT-87743). Remove the temporary
   Kotlin dev repository used for the BTA artifacts and resolve exclusively from Maven Central.
   Kotlin bootstrap releases are an acceptable non-expiring fallback only after verifying that
   the selected BTA implementation actually contains `ApplicationEnvironmentPin`.

   Bootstrap audit on 2026-08-04: `2.4.20-RC-197` is absent (API and implementation POMs both
   return 404), and every published bootstrap BTA implementation in the 2.4.20 and 2.5.0 lines
...
2. Monitor these three upstream issues when upgrading the BTA dependencies:
   ...
   - [KT-88183](https://youtrack.jetbrains.com/issue/KT-88183) — daemon compilation clears the
     compiler cache after each compilation; a fix could make the daemon flow viable again.
   - [KT-88278](https://youtrack.jetbrains.com/issue/KT-88278) — BTA tools use the host JVM's
     IntelliJ system properties. Once BTA accepts explicit per-session IntelliJ paths with safe
     defaults, delete `CodeEvalManager`'s `idea.config.path` pre-seed. Until then, the mitigation
     only redirects an unset property and never overwrites the host IDE's configured path.
